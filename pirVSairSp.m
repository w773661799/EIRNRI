...
for itsp = 1:length(spl)
  ...
  optionsP.Rel = Y; 
  optionsP.eps = orieps; 
  spPIR{itsp} = MC_PIRNN(X0,Xm,sp, lambda, mask, tol, optionsP); 

  % optionsP.eps = 10 ; 
  optionsP.Scalar = 0.8; 
  spAIR{itsp} = MC_AIRNN(X0,Xm,sp, lambda, mask, tol, optionsP); 
%   optionsP.alpha = 5e-1; 
%   spEPIR{itsp} = MC_EPIRNN(X0,Xm,sp, lambda, mask, tol, optionsP); 
end
  %%
