...
EPIR_peak_snr = zeros(100,1);%peak_snr
...
KLopt = 1e-4 ; 
Rank_EPIR = zeros(100,3) ; % 存入 PIR 的rank
...
for i =100
  ...
  for channel = 1:3
    ...
    opt.D_omega = omega_img(:,:,channel); % 观测矩阵
    
    Par = MC_EPIRNN(img_new(:,:,channel),opt.D_omega, opt.p, lambda/10, opt.omega, 1e-5, options);
